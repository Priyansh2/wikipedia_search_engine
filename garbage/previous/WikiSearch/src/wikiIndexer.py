...
class WikiHandler(xml.sax.handler.ContentHandler):
  ...
  def createIndex(self, title, text, infoBox, category, externalLink):    #add tokens generated to index
    ...
    vocabularyList = list(set(list(title.keys()) + list(text.keys()) + list(infoBox.keys()) + list(category.keys()) + list(externalLink.keys())))
...
def main():
    ...
    with open(OUTPUT_FOLDER + '/numberOfFiles.txt','w') as f:
      f.write(str(count))
