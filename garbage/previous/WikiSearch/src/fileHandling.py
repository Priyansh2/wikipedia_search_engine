...
def writeIntoFile(pathOfFolder, index, dict_Id, countFile, titleOffset):
    ...
    write_type = bz2.BZ2File(filename, 'wb', compresslevel=9) if COMPRESS_INDEX else open(filename, 'w')
    with write_type as f:
        f.write('\n'.join(data))
...
def mergeFiles(pathOfFolder, countFile):
    ...
    for i in range(countFile):
        ...
        indexFile[i] = bz2.BZ2File(fileName, 'rb') if COMPRESS_INDEX else open(fileName, 'r')
        flag[i] = 1
